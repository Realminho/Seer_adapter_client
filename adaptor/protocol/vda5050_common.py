...
class ErrorLevel(str, Enum):
    """VDA5050 error severity (current spec: four ascending levels).

    Canonical home for ErrorLevel. It lives here (version-neutral) rather than
    in the legacy ``vda5050_2_0_0_state`` module, whose older copy only defined
    WARNING/FATAL — that two-value version is the reason a v3 CRITICAL error was
    once impossible to express.
    """

    WARNING = "WARNING"  # No immediate attention; AGV continues order and takes new orders
    URGENT = "URGENT"  # Immediate attention (e.g. low battery); still drives and takes new orders
    CRITICAL = "CRITICAL"  # Must stop driving (can't continue order) but can still take new orders
    FATAL = "FATAL"  # User intervention required; can neither continue nor take new orders
