    ...
    def from_dict(cls, data: Dict):
        ...
        def parse_action(action_data: Dict) -> Action:
            ...
            for param in action_data.get("actionParameters", []):
                action_params.append(ActionParameter(
                    key=param["key"],
                    value=ActionParameterValue(param.get("value"))
                ))
            ...
            return Action(
                ...
                action_description=(
                    action_data.get("actionDescription")
                    or action_data.get("actionDescriptor")
                )
            )

        def edge_node_ids(edge_data: Dict, parsed_nodes: List[Node]) -> tuple[str, str]:
            ...
            previous_nodes = [
                node for node in parsed_nodes
                if node.sequence_id < edge_sequence_id
            ]
            next_nodes = [
                node for node in parsed_nodes
                if node.sequence_id > edge_sequence_id
            ...
            if not previous_nodes or not next_nodes:
                raise ValueError(
                    "v3 edge without startNodeId/endNodeId must be between two nodes by sequenceId"
                )
        ...
        for node_data in data.get("nodes", []):
            ...
            nodes.append(Node(
                ...
                node_description=(
                    node_data.get("nodeDescription")
                    or node_data.get("nodeDescriptor")
                ),
        ...
        for edge_data in data.get("edges", []):
            ...
            start_node_id, end_node_id = edge_node_ids(edge_data, nodes)
            ...
            edges.append(Edge(
                ...
                edge_description=(
                    edge_data.get("edgeDescription")
                    or edge_data.get("edgeDescriptor")
                ),
                max_speed=edge_data.get("maxSpeed") or edge_data.get("maximumSpeed"),
                max_height=(
                    edge_data.get("maxHeight")
                    or edge_data.get("maximumMobileRobotHeight")
                ),
                min_height=(
                    edge_data.get("minHeight")
                    or edge_data.get("minimumLoadHandlingDeviceHeight")
                ),
                ...
                rotation_allowed=edge_data.get("rotationAllowed") if "rotationAllowed" in edge_data else edge_data.get("reachOrientationBeforeEntering"),
