...
class SEER_commu(object):
    ...
    def __init__(self,ip="192.168.0.103"):
        ...
        self.state_socket.settimeout(30)
        # 상태 확인 소켓 통신 연결 진행
        ...
        self.task_socket.settimeout(30)
        ...
        self.ctrl_socket.settimeout(30)
    ...
    # 인자로 받은 landmark로 이동 명령을 보내는 함수 정의
    def gotarget_speed(self, landmark,method,max_speed):
        # 작업 명령 송신 소켓으로 인자로 받은 랜드마크로 이동 명령 전송
        self.task_socket.send(packMsg(1, robot_task_gotarget_req, {"id": landmark,"method": method, "max_speed": max_speed}))
    ...
    # 목표 방향, 속도를 인자로 받아서 지점까지 이동 후, 도착할 때까지 블로킹(대기)하는 함수 정의
    def gotarget_speed_block(self,point,method,max_speed):
        # gotarget 호출(클래스 내부 self 사용 아님)
        self.gotarget_speed(point,method,max_speed)
        print(f"[Nav info] {point}로 {method} 방향으로 최대 {max_speed} m/s로 주행을 시작합니다.")
        # 명령 전송 후 잠깐 대기
        time.sleep(0.5)
        # task_end()가 True가 될 때까지 반복 대기
        while self.task_end() is not True:
    ...
    def get_amr_node(self):
        ...
        payload = json.loads(data)
        # 현재 AMR 노드 좌표 저장
        amr_node = payload.get("current_station",None)
        # 데이터 return
        return amr_node         
