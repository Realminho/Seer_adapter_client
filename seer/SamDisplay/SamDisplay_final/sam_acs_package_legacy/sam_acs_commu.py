...
class SAM_ACS_AMR_Client:
    ...
    def extract_one_frame(self):
        ...
        if self.stx not in self.recv_queue:
            # 디버그 문구 print
            print("---------------------------------------------------------")
            print(f"[ACS CLIENT] 수신 버퍼에 STX X, ACS 명령 대기중")
            print("---------------------------------------------------------")
            # None return
            return None
        # 수신 버퍼에서 첫 STX 위치 찾기
    ...
    def send_s_command(self,target_node,carry_flag,move_flag):
        # S 명령 프레임 구성
        s_command_frame = self.s_command.build_s_command_frame(target_node=target_node,carry_flag=carry_flag,move_flag=move_flag)
        # 송신 결과 return
        return self.send_frame(s_command_frame)

    # S 명령 상태 보고 전용 함수 선언
    def send_s_status_report(self,target_node,carry_flag,move_flag):
        # 디버그 문구 print
        print("--------------------------------------------------")
        print(f"[ACS CLIENT] S Command 상태 보고 송신")
        print(f"[ACS CLIENT] target_node : {target_node}")
        print(f"[ACS CLIENT] carry_flag  : {carry_flag}")
        print(f"[ACS CLIENT] move_flag  : {move_flag}")
        print("--------------------------------------------------")
        # S 명령 송신 결과 return
        return self.send_s_command(target_node=target_node,carry_flag=carry_flag,move_flag=move_flag)
