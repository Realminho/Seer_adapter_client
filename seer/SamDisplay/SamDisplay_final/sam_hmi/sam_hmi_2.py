...
BATTERY_POS = (1350, 35)
BATTERY_SIZE = (185, 165)
# MoMa State AMR 아이콘 출력 위치 및 크기 설정
AMR_ICON_POS = (115, 280)
AMR_ICON_SIZE = (190, 170)
# Push 아이콘 출력 위치 및 크기 설정
PUSH_ICON_POS = (95, 575)
PUSH_ICON_SIZE = (210, 165)
# Battery Info 아이콘 출력 위치 및 크기 설정
BATTERY_2_POS = (980, 270)
BATTERY_2_SIZE = (130, 110)
# ----------------------- 텍스트 위치 설정 -----------------------
# 메인 제목 중심 좌표 설정
TITLE_CENTER_POS = (800, 125)
# MoMa State 제목 중심 좌표 설정
MOMA_STATE_TITLE_CENTER_POS = (515, 320)
# Release Brake 제목 중심 좌표 설정
RELEASE_BRAKE_TITLE_CENTER_POS = (525, 610)
# Battery Info 제목 중심 좌표 설정
BATTERY_INFO_TITLE_CENTER_POS = (1265, 325)
# ----------------------- 박스 및 버튼 위치 설정 -----------------------
# MoMa State 상태 표시 박스 설정
MOMA_STATE_BOX_RECT = pygame.Rect(315, 365, 520, 120)
# 좌측 영역 구분선 시작/끝 좌표 설정
LEFT_DIVIDER_START_POS = (95, 520)
LEFT_DIVIDER_END_POS = (860, 520)
# ON 버튼 위치 및 크기 설정
ON_BUTTON_RECT = pygame.Rect(335, 665, 240, 120)
# OFF 버튼 위치 및 크기 설정
OFF_BUTTON_RECT = pygame.Rect(615, 665, 240, 120)
# Battery Info 내용 표시 박스 설정
BATTERY_INFO_BOX_RECT = pygame.Rect(970, 405, 525, 365)
# ----------------------- 버튼 클릭 영역 딕셔너리 설정 -----------------------
# 버튼 Rect 딕셔너리 설정
BUTTON_RECTS = {
...
def draw_empty_info_box(screen, rect, radius=18):
    # 흰색 박스 출력
    pygame.draw.rect(screen, PURE_WHITE, rect, border_radius=radius)
    # 파란색 테두리 출력
    pygame.draw.rect(screen, BORDER_BLUE, rect, width=2, border_radius=radius)
...
class L2M_HMI_Controller:
    ...
    def get_nav_state_text(self):
        ...
        with self.nav_lock:
            ...
            # Run Completed 시작 시간 복사
            run_completed_start_time = self.run_completed_start_time
        # 현재 표시 상태가 Run Completed이고 시작 시간이 있으면
    ...
    def set_release_brake(self, flag):
        ...
        if self.dio is None:
            # 디버그 출력
            print("[HMI] DIO 모듈이 연결되지 않아 Release Brake를 제어할 수 없습니다.")
            # 실패 반환
            return False
        # 에러가 없으면
        try:
            ...
            with self.dio_lock:
                ...
                if flag:
                    # DO31 High
                    self.dio.set_do_on(RELEASE_BRAKE_DO)
                    # 디버그 출력
                    print("[HMI] Release Brake ON : DO31=High")
                # flag가 False면
                else:
                    # DO31 Low
                    self.dio.set_do_off(RELEASE_BRAKE_DO)
                    # 디버그 출력
...
def main():
    ...
    fonts = {
        ...
        "section_title": get_korean_font(48, bold=True),
        "button": get_korean_font(48, bold=True),
        "top_battery": get_korean_font(24, bold=True),
        "moma_state": get_korean_font(44, bold=True),
        "battery_info_title": get_korean_font(30, bold=True),
        "battery_info_row": get_korean_font(24, bold=True),
