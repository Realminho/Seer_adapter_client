...
DOBOT_ON_BUTTON_RECT = scaled_rect(1205, 60, 175, 145)
# ----------------------- 버튼 클릭 영역 딕셔너리 설정 ------------------
# 버튼 Rect 딕셔너리 설정
BUTTON_RECTS = {
...
# Dobot Off 신호 유지 시간 설정
DOBOT_OFF_PULSE_SEC = 3.5
# 배터리 95% 이상 도달 시 제어할 DIO DO 번호 설정
BATTERY_FULL_SIGNAL_DO = 14
# 배터리 95% 이상 도달 시 DO High 유지 시간 설정
BATTERY_FULL_SIGNAL_HIGH_SEC = 1.0
# 배터리 95% 이상 도달 시 DO Pulse 반복 횟수 설정
BATTERY_FULL_SIGNAL_REPEAT_COUNT = 5
# 배터리 95% 이상 도달 시 DO Pulse 반복 사이 대기 시간 설정
BATTERY_FULL_SIGNAL_INTERVAL_SEC = 0.2
# ----------------------- SEER AMR 설정 -----------------------
# SEER AMR IP 설정
...
# ----------------------- 배터리 / 네비게이션 정보 갱신 설정 -----------------------
# 배터리 정보 갱신 주기 설정
BATTERY_UPDATE_INTERVAL_SEC = 10.0
# 네비게이션 상태 갱신 주기 설정
...
# ----------------------- Dobot On / Off 원형 버튼 출력 함수 선언 -----------------------
def draw_dobot_button(screen, rect, font, button_mode_text):
    # 버튼 중심 좌표 계산
    center_pos = rect.center
    ...
    text_surface_2 = font.render(button_mode_text, True, PURE_WHITE)
...
class L2M_HMI_Controller:
    ...
    def __init__(self):
        ...
        # Dobot On / Off 상태 보호를 위한 Lock 선언
        self.dobot_state_lock = threading.Lock()
        ...
        # Dobot 버튼 현재 상태 저장 변수 선언
        # False이면 화면에 Dobot On 표시
        # True이면 화면에 Dobot Off 표시
        self.dobot_is_on = False

        # Dobot 버튼 명령 실행 중 여부 저장 변수 선언
        self.dobot_command_running = False

        # 배터리 95% 이상 도달 신호가 이미 실행되었는지 저장하는 변수 선언
        self.battery_full_signal_done = False

        # 배터리 95% 이상 도달 신호 실행 중 여부 저장 변수 선언
        self.battery_full_signal_running = False
                ...
                try:
                    ...
                    if isinstance(battery_info, tuple) and len(battery_info) >= 9:
                        ...
                        if current_battery_level is not None:
                            ...
                            # 배터리 레벨이 95% 이상이고 아직 신호를 실행하지 않았으면
                            if display_battery_level >= 95 and not self.battery_full_signal_done:
                                # 중복 Thread 생성을 막기 위해 실행 완료 플래그를 먼저 True로 변경
                                self.battery_full_signal_done = True

                                # 로그 출력
                                print("[HMI] 배터리 95% 이상 감지 -> DO14 1초 High 후 Low 5회 반복 시작")

                                # DO14 Pulse를 비동기로 실행
                                self.pulse_battery_full_signal_async()

                            # 배터리 레벨이 95% 미만으로 내려가면 다음 95% 도달 시 다시 실행 가능하도록 초기화
                            elif display_battery_level < 95:
                                self.battery_full_signal_done = False
    ...
    # 현재 Dobot 버튼 표시 문구 반환 함수 선언
    def get_dobot_button_mode_text(self):
        with self.dobot_state_lock:
            if self.dobot_is_on:
                return "Off"
    ...
    # 배터리 95% 이상 도달 DO Pulse 비동기 실행 함수 선언
    def pulse_battery_full_signal_async(self):
        # 배터리 95% 이상 도달 DO Pulse Thread 생성
        battery_full_signal_thread = threading.Thread(
            target=self.pulse_battery_full_signal,
            daemon=True
        )

        # 배터리 95% 이상 도달 DO Pulse Thread 시작
        battery_full_signal_thread.start()

    # 배터리 95% 이상 도달 DO Pulse 제어 함수 선언
    def pulse_battery_full_signal(self):
        # 임시 DIO 객체 변수 선언
        temp_dio = None
        ...
        try:
            # 이미 배터리 95% 이상 신호가 실행 중이면 중복 실행 방지
            if self.battery_full_signal_running:
                print("[HMI] 배터리 95% 이상 도달 신호가 이미 실행 중입니다.")
                return False

            # 실행 중 상태로 변경
            self.battery_full_signal_running = True
            ...
            with self.dio_lock:
                ...
                # 지정한 횟수만큼 DO High 후 Low 반복
                for repeat_index in range(BATTERY_FULL_SIGNAL_REPEAT_COUNT):
                    # DO14 High 출력
                    temp_dio.set_do_on(BATTERY_FULL_SIGNAL_DO)
                    ...
                    print(
                        f"[HMI] 배터리 95% 이상 신호 "
                        f"{repeat_index + 1}/{BATTERY_FULL_SIGNAL_REPEAT_COUNT} : "
                        f"DO{BATTERY_FULL_SIGNAL_DO}=High"
                    )

                    # 지정한 시간 동안 High 유지
                    time.sleep(BATTERY_FULL_SIGNAL_HIGH_SEC)

                    # DO14 Low 출력
                    temp_dio.set_do_off(BATTERY_FULL_SIGNAL_DO)
                    ...
                    print(
                        f"[HMI] 배터리 95% 이상 신호 "
                        f"{repeat_index + 1}/{BATTERY_FULL_SIGNAL_REPEAT_COUNT} : "
                        f"DO{BATTERY_FULL_SIGNAL_DO}=Low"
                    )

                    # 마지막 반복이 아니라면 짧게 대기
                    if repeat_index < BATTERY_FULL_SIGNAL_REPEAT_COUNT - 1:
                        time.sleep(BATTERY_FULL_SIGNAL_INTERVAL_SEC)
            ...
            # 에러 출력
            print(f"[HMI] 배터리 95% 이상 DO Pulse 제어 실패 : {error}")
            return False

        finally:
            # High 상태에서 에러가 발생한 경우 Low 복구 시도
            if temp_dio is not None and do_high_done:
                try:
                    temp_dio.set_do_off(BATTERY_FULL_SIGNAL_DO)
                    print(f"[HMI] 배터리 95% 이상 예외 복구 : DO{BATTERY_FULL_SIGNAL_DO}=Low")
                except Exception as error:
                    print(f"[HMI] 배터리 95% 이상 예외 복구 실패 : {error}")

            # DIO 연결 종료
            if temp_dio is not None:
                try:
                    temp_dio.dio_close()
                    print("[HMI] 배터리 95% 이상 신호 제어 후 DIO 연결 종료")
                except Exception as error:
                    print(f"[HMI] DIO 연결 종료 실패 : {error}")
    ...
    # Dobot On / Off DO Pulse 비동기 실행 함수 선언
    def pulse_dobot_async(self):
        dobot_thread = threading.Thread(
            target=self.pulse_dobot,
            daemon=True
        )

        dobot_thread.start()

    # Dobot On / Off DO Pulse 제어 함수 선언
    def pulse_dobot(self):
        # 임시 DIO 객체 변수 선언
        temp_dio = None

        # Dobot 출력 High 성공 여부 변수 선언
        do_high_done = False

        # Dobot 실행할 명령 정보 변수 선언
        command_text = None
        pulse_sec = None
        next_dobot_state = None

        try:
            # Dobot 상태 Lock으로 현재 버튼 상태 확인
            with self.dobot_state_lock:
                # 이미 Dobot 명령이 실행 중이면 중복 실행 방지
                if self.dobot_command_running:
                    print("[HMI] Dobot 명령이 이미 실행 중입니다.")
                    return False

                # Dobot 명령 실행 중 상태로 변경
                self.dobot_command_running = True

                # 현재 화면이 Dobot On 상태이면 On Pulse 실행
                if not self.dobot_is_on:
                    command_text = "On"
                    pulse_sec = DOBOT_ON_PULSE_SEC
                    next_dobot_state = True

                # 현재 화면이 Dobot Off 상태이면 Off Pulse 실행
                else:
                    command_text = "Off"
                    pulse_sec = DOBOT_OFF_PULSE_SEC
                    next_dobot_state = False
            ...
            with self.dio_lock:
                ...
                # 로그 출력
                print(f"[HMI] Dobot {command_text} : DO{DOBOT_ON_DO}=High")
                ...
                # 로그 출력
                print(f"[HMI] Dobot {command_text} : DO{DOBOT_ON_DO}=Low")

            # Pulse가 정상 완료되면 버튼 상태 변경
            with self.dobot_state_lock:
                self.dobot_is_on = next_dobot_state
            ...
            # 에러 출력
            print(f"[HMI] Dobot {command_text} DO Pulse 제어 실패 : {error}")
            return False
            ...
            if temp_dio is not None and do_high_done:
                try:
                    temp_dio.set_do_off(DOBOT_ON_DO)
                    print(f"[HMI] Dobot {command_text} 예외 복구 : DO{DOBOT_ON_DO}=Low")
                except Exception as error:
                    print(f"[HMI] Dobot {command_text} 예외 복구 실패 : {error}")

            # DIO 연결 종료
            if temp_dio is not None:
                try:
                    temp_dio.dio_close()
                    print(f"[HMI] Dobot {command_text} 제어 후 DIO 연결 종료")
                except Exception as error:
                    print(f"[HMI] DIO 연결 종료 실패 : {error}")
...
def draw_ui(screen, fonts, ui_images, hmi_controller):
    ...
    # Dobot 버튼 표시 문구 가져오기
    dobot_button_mode_text = hmi_controller.get_dobot_button_mode_text()
    ...
    draw_dobot_button(
        ...
        button_mode_text=dobot_button_mode_text
    )
...
def handle_mouse_down(mouse_pos, hmi_controller):
    ...
    # Dobot On / Off 버튼 클릭 시
    elif BUTTON_RECTS["DOBOT_ON_BUTTON"].collidepoint(mouse_pos):
        dobot_button_mode_text = hmi_controller.get_dobot_button_mode_text()

        if dobot_button_mode_text == "On":
            print(f"[HMI] Dobot On 버튼 클릭 -> DO{DOBOT_ON_DO} {DOBOT_ON_PULSE_SEC}초 High 후 Low")
        else:
            print(f"[HMI] Dobot Off 버튼 클릭 -> DO{DOBOT_ON_DO} {DOBOT_OFF_PULSE_SEC}초 High 후 Low")

        hmi_controller.pulse_dobot_async()
