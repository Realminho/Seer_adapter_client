...
from side_docking_package.aruco_dock_only_yaw import Aruco_Dock_Commu_Only_Yaw
...
def main():
    ...
    answer_path = "/home/mic-711/Desktop/SamDisplay/side_docking_package/aruco_answer_json/aruco_2.json"
    ...
    docker = Aruco_Dock_Commu_Only_Yaw(seer=seer,detector=detector,answer=answer,
        # yaw 허용 오차[deg]
    ...
    try:
        ...
        dock_ok = docker.align_to_answer_marker(
            ...
            path=answer_path)
        # 도킹 시간 측정 종료
        dock_end_time = time.time()
        # 도킹 소요 시간 계산
        dock_elapsed_time = dock_end_time - dock_start_time
        # 도킹 실패 시
        if not dock_ok:
            # 도킹 실패 로그 출력
            print("[AMR] 도킹 실패")
        # 도킹 성공 시
            ...
            print("[AMR] 도킹 성공")
            # 도킹 소요 시간 출력
            print(f"[AMR] 도킹 소요 시간 : {dock_elapsed_time:.3f}초")
        # 도킹 후 안정화를 위해 잠시 대기
        time.sleep(1.0)
