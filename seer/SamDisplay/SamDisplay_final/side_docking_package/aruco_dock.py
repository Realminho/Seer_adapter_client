...
# =============================================================
class Aruco_Dock_Commu:
    # 클래스 초기화 함수 선언 
    def __init__(self,seer,detector,answer,yaw_tolerance_deg=1.0,x_tolerance_m=0.01,yaw_direction_sign=1.0,move_direction_sign=1.0):
        # AMR 제어 객체 저장 변수 선언
        self.seer = seer
    ...
    def align_to_answer_marker(self, max_step=300, search_w=0.06, show_window=True,path=None):
        ...
        # 도킹에 필요한 필수 정답 정보(마커 각도, 마커 X 좌표) 중 하나라도 없다면
        if target_id is None or target_yaw_deg is None or target_marker_x is None:
            # 정답 정보 부족 로그 출력
        ...
        # 정답 marker_x 값 출력
        print(f"[ARUCO_DOCK] target_marker_x : {target_marker_x}")
        # 정답 marker_z 값 출력
        print(f"[ARUCO_DOCK] target_marker_z : {target_marker_z}")
        # 최대 보정 횟수만큼 반복합니다.
        for step in range(max_step):
            ...
            # 현재 marker_x 값을 가져옵니다.
            current_marker_x = current_marker.get("marker_x")
            # 현재 marker_z 값을 가져옵니다.
            current_marker_z = current_marker.get("marker_z")
            # 현재 offset_px 값을 가져옵니다.
            current_offset_px = current_marker.get("offset_px")
            # 현재 yaw 또는 marker_x가 없다면
            if current_yaw_deg is None or current_marker_x is None:
                # 계산 실패 로그를 출력
                print(f"[ARUCO_DOCK] step={step + 1}, yaw 또는 marker_x 계산 실패")
                # 다음 반복으로 넘어가기
                continue
            ...
            # 현재 marker_x와 정답 marker_x의 차이를 계산
            x_error_m = current_marker_x - float(target_marker_x)
            # yaw 오차가 허용 범위 안인지 확인
            yaw_ok = abs(yaw_error_deg) <= self.yaw_tolerance_deg
            # marker_x 오차가 허용 범위 안인지 확인
            x_ok = abs(x_error_m) <= self.x_tolerance_m
            # 로그 구분선 출력
            print("-" * 60)
            # 현재 step 번호 출력
            ...
            # 현재 marker_x 값 출력
            print(f"[ARUCO_DOCK] current_marker_x : {current_marker_x:.4f}")
            # 정답 marker_x 값 출력
            print(f"[ARUCO_DOCK] target_marker_x  : {target_marker_x:.4f}")
            # marker_x 오차 출력
            print(f"[ARUCO_DOCK] x_error_m        : {x_error_m:.4f}")
            # 현재 marker_z 값 출력
            print(f"[ARUCO_DOCK] current_marker_z : {current_marker_z}")
            # 정답 marker_z 값 출력
            print(f"[ARUCO_DOCK] target_marker_z  : {target_marker_z}")
            # 현재 offset_px 값 출력
            print(f"[ARUCO_DOCK] current_offset_px: {current_offset_px}")
            ...
            # yaw가 허용 범위를 벗어났다면
            if not yaw_ok:
                # yaw 오차에 비례하여 회전 속도 계산 (각도 보정 속도 변경 가능)
                ...
                # 다음 반복으로 넘어가기
                continue
            # marker_x가 허용 범위를 벗어났다면
            if not x_ok:
                # marker_x 오차에 비례하여 전진/후진 속도 계산 (X축 보정 속도 변경 가능)
                vx_cmd = 0.8 * x_error_m
                # 전진/후진 방향 적용
                vx_cmd *= self.move_direction_sign
                # 전진/후진 속도를 최소/최대 범위로 제한
                vx_cmd = self.clamp_signed(vx_cmd, min_abs=0.025, max_abs=0.1)
                # 전진/후진 보정 로그 출력
                print(f"[ARUCO_DOCK] 전후 위치 보정 -> vx={vx_cmd:.3f}")
                # AMR에 전진/후진 명령 전송
                self.seer.motion_control(
                    vx=vx_cmd,
                    vy=0.0,
